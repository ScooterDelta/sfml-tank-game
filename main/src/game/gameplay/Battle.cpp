...
namespace game::gameplay {
  ...
  Battle::Battle(Vector2D screenDimensions) :
    ...
    _missileTimer1{high_resolution_clock::now()},
    _missileTimer2{high_resolution_clock::now()},
    _mineTimer1{high_resolution_clock::now()},
    _mineTimer2{high_resolution_clock::now()},
    _score{0, 0} {
  ...
  void Battle::update() {
    ...
    while (turretIter != _turrets.end()) {
      if ((*turretIter)->canFollowTarget(_tank1, _tank1.isInvisible()) && (*turretIter)->canFireMissile()) {
        std::unique_ptr < Missile > newMissile(
                                               new Missile{(*turretIter)->getFirePosition(), (*turretIter)->getDirection(), Score::NONPLAYER});
        _missiles.push_back(std::move(newMissile));
      } else if ((*turretIter)->canFollowTarget(_tank2, _tank2.isInvisible()) &&
                 (*turretIter)->canFireMissile()) {
        std::unique_ptr < Missile > newMissile(
                                               new Missile{(*turretIter)->getFirePosition(), (*turretIter)->getDirection(), Score::NONPLAYER});
        _missiles.push_back(std::move(newMissile));
      } else
    ...
    while (_explosionIterator != _explosions.end()) {
      if (duration_cast<milliseconds>( high_resolution_clock::now() - (*_explosionIterator)->getExplosionTime() ).count() > 90)
  ...
  bool Battle::checkIsHorizontal(Vector2D &point, AxisAligned &obstacle) {
    ...
    if (leftDistance < bottomDistance && leftDistance < topDistance && leftDistance < rightDistance)
      return false;
    else return !(rightDistance < bottomDistance && rightDistance < topDistance && rightDistance < leftDistance);
  }
  ...
  bool Battle::isFrontWallCollision(Tank &tank, bool &isHorizontal) {
    // Check for a collision with a wall in front of the tank.
